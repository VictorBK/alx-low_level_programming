 ...
 * @error: the number is either exit value or file descriptor
 * @s: str is the name of either of the two filenames
 * @fd: file descriptor
 * Return: 0 (success)
 */
...
int __exit(int error, char *s, int fd)
{
	switch (error)
	{
		...
		case 98:
			...
			exit(error);
		case 99:
			dprintf(STDERR_FILENO, "Error: Can't write to fie %s\n", s);
			exit(error);
		case 100:
...
int main(int argc, char *argv[])
{
	...
	/*setting the file descriptor for copy-to file*/
	fd_2 = open(argv[2], O_CREAT | O_TRUNC | O_WRONLY, 0664);
	if (fd_2 == -1)
		__exit(99, argv[2], 0);

	/*setting the file descriptor for copy-from file*/
	fd_1 = open(argv[1], O_RDONLY);
	if (fd_1 == -1)
		__exit(98, argv[1], 0);

	/*reading the original file as long as there is more than 0 to read*/
